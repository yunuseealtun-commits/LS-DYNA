    ...
    const VanillaQuillEditor = ({ value, onChange, placeholder, modules, className, autoFocus }) => {
      ...
      useEffect(() => {
        ...
        if (!quillInstance.current) {
          ...
          if (autoFocus) {
            setTimeout(() => quillInstance.current.focus(), 100);
          }
        }

        // Prevent cursor jumping while typing by only updating if content heavily mismatches (like tab switch)
        const currentHtml = editorRef.current.querySelector('.ql-editor').innerHTML;
        if (value !== undefined && value !== currentHtml) {
          // Avoid overwriting a '<p><br></p>' with empty string and causing selection reset
          if (value === '' && currentHtml === '<p><br></p>') return;
          quillInstance.current.clipboard.dangerouslyPasteHTML(value);
        }
      }, [value, modules, autoFocus]);
