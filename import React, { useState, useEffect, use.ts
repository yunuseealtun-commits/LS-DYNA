...
const DraggableWindow = ({ title, children, onClose, initialPos, zIndex, onFocus, width = 450, height, minHeight = 200, maxHeight = 800 }) => {
  ...
  return (
    <div 
      className= "absolute flex flex-col bg-[#f0f0f0] border-2 border-white border-r-gray-500 border-b-gray-500 shadow-xl"
  style = {{ left: pos.x, top: pos.y, zIndex, width, height, minHeight, maxHeight }
}
onMouseDown = { onFocus }
  >
  <div 
  ...
  <span className="font-bold tracking-wide text-sm flex items-center gap-2" >
    ...
    < div className = "flex gap-1" onPointerDown = { e => e.stopPropagation() } >
      <button className="w-4 h-4 bg-[#c0c0c0] border border-white border-r-gray-600 border-b-gray-600 flex items-center justify-center text-black font-bold text-xs leading-none" > _ </button>
        < button className = "w-4 h-4 bg-[#c0c0c0] border border-white border-r-gray-600 border-b-gray-600 flex items-center justify-center text-black font-bold text-xs leading-none" > <Square size={ 8 } /></button >
          { onClose && (
            <button onClick={ onClose } className = "w-4 h-4 bg-[#c0c0c0] border border-white border-r-gray-600 border-b-gray-600 flex items-center justify-center text-black font-bold text-xs leading-none" >
              <X size={ 12 } />
                </button>
          )}
</div>
...
const KeywordManager = () => {
  ...
  const [openForms, setOpenForms] = useState([]); // { id, node, pos: {x,y}, zIndex }
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/data')
      ...
      .catch(err => console.log('DB Load Info:', err.message));
  }, []);
  ...
  const saveDatabase = () => {
    ...
    fetch('http://localhost:8000/api/data', {
      ...
      body: JSON.stringify({ treeData, parameterNotes })
    })
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => {
        setSaveMessage('Error Saving DB');
        setTimeout(() => setSaveMessage(''), 3000);
      })
  ...
  const TreeItem = ({ item, level = 0 }) => {
    ...
    onClick = {(e) => {
  if (e.detail === 2) {
    if (item.type === 'group') toggleNode(item.id);
            ...
            <span className="font-bold text-gray-600 text-right pr-1 self-end mb-1" > { rowIdx + 1} </span>
              ...
              <div key= {`${rowIdx}-${colIdx}`} className = "flex flex-col gap-0.5 w-full" >
              {
                field.label ? (
                  <>
                  <span 
                      className= "text-blue-700 underline cursor-pointer hover:text-blue-900 text-[10px] truncate" 
                      title={ field.label } 
                      onClick={() => setSelectedParameter(field.label)
              }
onDoubleClick = {() => {
  setSelectedParameter(field.label);
  setTempNote(parameterNotes[field.label] || '');
...
{/* Bottom Text Area */ }
<div className="mt-2 h-[120px] border border-gray-400 bg-white shadow-inner p-2 overflow-y-auto font-sans shrink-0 text-[12px] relative group" >
{
  selectedParameter?(
    <>
  <div 
        className="absolute top-0 right-0 px-2 py-1 bg-gray-200 border-l border-b border-gray-400 text-[10px] font-bold cursor-pointer hover:bg-gray-300 opacity-0 group-hover:opacity-100 transition-opacity"
onClick = {() => { setTempNote(parameterNotes[selectedParameter] || ''); setIsEditorOpen(true); }}
      > Edit Note </div>
  < div className = "font-bold text-[#000080] mb-1 pb-1 border-b border-gray-300" > { selectedParameter } Notes: </div>
{
  parameterNotes[selectedParameter] ? (
    <div className= "prose prose-sm max-w-none" dangerouslySetInnerHTML = {{ __html: parameterNotes[selectedParameter] }
} />
      ) : (
  <div className= "text-gray-500 italic" > No notes created yet.Double - click the parameter or click 'Edit Note' to add one.</div>
      )}
</>
  ) : (
  <div className= "text-gray-500 italic font-mono" > $# Select a parameter to view or edit its notes...</div>
  )}
</div>
  </div>
    );
...
<button 
                className="px-2 bg-[#c0c0c0] border border-white border-r-gray-600 border-b-gray-600 text-[10px] py-[1px] active:border-gray-600 active:border-r-white active:border-b-white hover:bg-[#d0d0d0] font-bold shadow-sm"
...
{/* React Quill Editor Modal */ }
{
  isEditorOpen && selectedParameter && (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]" >
      <div className="bg-[#f0f0f0] border-2 border-white border-r-gray-600 border-b-gray-600 shadow-xl w-[600px] h-[500px] flex flex-col pt-1" >
        <div className="bg-gradient-to-r from-[#000080] to-[#1084d0] px-2 py-1 mx-1 flex justify-between items-center text-white cursor-default select-none" >
          <span className="font-bold tracking-wide text-sm flex items-center gap-2" >
            <FileText size={ 14 } /> Editing Note: {selectedParameter}
              </span>
              < button className = "w-4 h-4 bg-[#c0c0c0] border border-white border-r-gray-600 border-b-gray-600 flex items-center justify-center text-black font-bold text-xs" onClick = {() => setIsEditorOpen(false)
}> <X size={ 12 } /></button >
  </div>
  < div className = "p-2 flex-1 flex flex-col bg-white m-2 border border-gray-400 shadow-inner" >
    <ReactQuill 
          theme="snow"
value = { tempNote }
onChange = { setTempNote }
className = "h-[360px] mb-10"
modules = {{
  toolbar: [
    [{ 'header': [1, 2, 3, false] }],
  ...
  < div className = "p-2 flex justify-end gap-2 bg-[#f0f0f0] mx-1 border-t border-gray-400" >
    <button 
          className="px-4 py-1 bg-[#c0c0c0] border-2 border-white border-r-gray-600 border-b-gray-600 active:border-gray-600 active:border-r-white active:border-b-white"
onClick = {() => setIsEditorOpen(false)}
        >
  Cancel
  </button>
  < button
className = "px-4 py-1 bg-[#c0c0c0] border-2 border-white border-r-gray-600 border-b-gray-600 active:border-gray-600 active:border-r-white active:border-b-white font-bold"
onClick = {() => {
  setParameterNotes(prev => ({ ...prev, [selectedParameter]: tempNote }));
